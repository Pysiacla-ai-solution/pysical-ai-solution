...
async def submit_job(
    ...
    user_id: str = "anonymous",  # Fair-share 계산의 기준이 됨
    vram_required: int = 2 * 1024**3,  # Job Size 계산의 기준이 됨
    # urgency 삭제됨 -> Partition과 QOS로 대체
    partition: Literal["debug", "normal", "batch"] = "normal",
    qos: Literal["admin", "premium", "standard", "guest"] = "standard",
    scheduler: GpuScheduler = Depends(get_scheduler),
) -> Job:
    """
    Python 스크립트 업로드 후 SLURM 스타일 우선순위 큐에 등록.
    
    - **partition**: 작업의 성격 (debug: 높은 우선순위, batch: 낮은 우선순위)
    - **qos**: 사용자의 등급 (admin: 최우선, guest: 최하위)
    """
    ...
    # Scheduler의 submit_job 호출 시 변경된 파라미터 전달
    job = await scheduler.submit_job(
        src_script=temp_path,
        user_id=user_id,
